...
public struct OpenWeatherLocationData {
    ...
    init(location: CLLocation, eventTime: Date? = nil) {
        let timestampDate = eventTime ?? location.timestamp
        self.init(
            timestamp: Int64(timestampDate.timeIntervalSince1970 * 1000),
...
final class OpenWeatherLocationProvider: NSObject, CLLocationManagerDelegate {
    ...
    private func publishLatestLocation(eventTime: Date? = nil) {
        guard let location = lastLocation ?? locationManager.location else {
            if debug { print(OpenWeatherSensor.TAG, "Location data is lost") }
            return
        }
...
public class OpenWeatherSensor: AwareSensor {

    
    public var CONFIG:Config = Config()
    var locationProvider:OpenWeatherLocationProvider?
    ...
    public func onLocationChanged(data location: OpenWeatherLocationData) {
    ...
    private func initializeTable() {
        ...
        do {
            try OpenWeatherData.createTable(queue: queue)
        } catch {
            if self.CONFIG.debug { print(error) }
        }
    }
